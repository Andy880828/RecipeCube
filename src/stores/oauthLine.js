...
export const useOAuthLineStore = defineStore('oauthLine', {
    ...
    actions: {
                            ...
                            try {
                                ...
                                if (userData.ExclusiveChecked) {
                                    const exclusiveIngredientsResponse = await fetch(
                                        `${import.meta.env.VITE_API_BASEURL}/UserIngredients/ExclusiveIngredientsName?User_Id=${userData.UserId}`
                                    );
                                    ...
                                    if (
                                        ...
                                        const exclusiveIngredientsFormatted =
                                            exclusiveIngredientsGet.exclusiveIngredients
                                                .map(
                                                    (ingredient) =>
                                ...
                                if (userData.PreferredChecked) {
                                    const preferredIngredientsResponse = await fetch(
                                        `${import.meta.env.VITE_API_BASEURL}/UserIngredients/PreferedIngredientsName?User_Id=${userData.UserId}`
                                    );
        ...
        handleLineLogin() {
            if (!liff.isLoggedIn()) {
                liff.login({ redirectUri: 'https://chlinandys.com/' });
            }
        },
        async oAuthFirstSignIn(Vegetarianrestrictions) {
            ...
            if (oAuthLineUserEmail != null) {
                ...
                const response = await fetch(API_URL, {
                    method: 'PUT',
                    body: JSON.stringify({
                        oAuthEmail: oAuthLineUserEmail.value,
